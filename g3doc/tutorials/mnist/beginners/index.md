...
이것이 소프트맥스 회귀가 자연스럽고 간단한 모델로 있을 때의 전형적인 경우입니다. 만약 당신이 여러가지 중 하나에 확률을 부여하고자 한다면, 소프트맥스가 그걸 해줍니다. 뒤에 가서 우리가 좀 더 복잡한 모델을 학습시킬 때조차, 맨 마지막 단계는 소프트맥스 레이어가 될 것입니다.
